...
typedef struct
{
  ...
  TCHAR *folder[FOLDER_NUM]; 
  TCHAR *file[FILE_NUM];     
  uint16_t F_num;            
  uint16_t f_num;            
  uint16_t cur_page;         
  FS_SOURCE source;          
  TCHAR *Longfile[FILE_NUM]; 
  TCHAR pageTitle[40];       
} MACROFILE;
    ...
    switch (key_num)
    {
      ...
      default:
        if (key_num <= KEY_ICON_4)
        {
          ...
          if (key_num + start < macroFile.F_num) 
          {
          ...
          else if (key_num + start < macroFile.F_num + macroFile.f_num) 
          {
...
void macroListDraw(void)
{
  ...
  */
  ...
  for (i = 0; (i + macroFile.cur_page * NUM_PER_PAGE < macroFile.F_num) && (i < NUM_PER_PAGE); i++) 
  {
  ...
  for (; (i + macroFile.cur_page * NUM_PER_PAGE < macroFile.f_num + macroFile.F_num) && (i < NUM_PER_PAGE); i++) 
  {
