...
bool powerFailedCreate(char *path)
{
  ...
  create_ok = false;
  if (!infoSettings.powerloss_en) return false;   
  if (infoFile.source >= BOARD_SD) return false;  
...
void powerFailedCache(u32 offset)
{
  ...
  if (infoBreakPoint.axis[Z_AXIS] == coordinateGetAxisTarget(Z_AXIS)) return;  
  ...
  { 
    ...
    infoBreakPoint.speed = speedGetCurPercent(0);  
    infoBreakPoint.flow = speedGetCurPercent(1);  
  ...
  else if (infoBreakPoint.pause) return;  
