...
uint32_t ulMediaDetectTimeout = 0;	// Flag to set USB/SD insertion popup dialog
...
// Function to check the storage media events (Insert/Removal)
// and intiate timeout sequence for displaying files and popup dialog
void wCheckStorageMediaEvents(void)
{
  if(volumeSrcStatus[TFT_SD] && !infoHost.printing)
  {
    infoFile.source = TFT_SD;
    ulMediaDetectTimeout = 1;
    resetInfoFile();
    powerFailedSetDriverSource(getCurFileSource());
  }
  else if(volumeSrcStatus[TFT_UDISK] && !infoHost.printing)
  {
...
void loopVolumeSource(void) // Checks for newly inserted or removed volume (SD, USB, etc)
{
  for (uint8_t i = 0; i < FF_VOLUMES; i++)
  {
    if (volumeSrcStatus[i] != (*volumeInserted[i])())  // WUXN comment: this means there has been a change in the card insert status
    {
