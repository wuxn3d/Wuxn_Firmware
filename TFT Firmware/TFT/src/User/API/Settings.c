...
void infoSettingsReset(void)
{
  ...
  #ifdef WUXN_CODE
  infoSettings.enable_oobwcw          = ENABLED;
  #endif
  ...
  infoSettings.pause_pos[X_AXIS]      = NOZZLE_PAUSE_X_POSITION;  
  infoSettings.pause_pos[Y_AXIS]      = NOZZLE_PAUSE_Y_POSITION;  
  ...
  infoSettings.factory_reset          = DISABLED;
  #endif
  ...
  for (int i = 0; i < FEEDRATE_COUNT - 1 ; i++)  
  {
  ...
  {
    infoSettings.pause_feedrate[i]    = default_pause_speed[i];  
  }
