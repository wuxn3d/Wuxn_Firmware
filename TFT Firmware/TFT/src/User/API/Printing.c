...
void preparePrintSummary(void)
{
  ...
  {
    infoPrintSummary.length = (infoPrintSummary.length * speedGetCurPercent(1)) / 100;  
    infoPrintSummary.weight = (infoPrintSummary.weight * speedGetCurPercent(1)) / 100;  
    infoPrintSummary.cost   = (infoPrintSummary.cost   * speedGetCurPercent(1)) / 100;  
  }
}
...
void updateFilamentUsed(void)
{
  ...
  if ((E_pos + MAX_RETRACT_LIMIT) < last_E_pos)  
  {
    last_E_pos = 0;
  }
...
void getGcodeFromFile(void)
{
  ...
  for (; infoPrinting.cur < infoPrinting.size;)
  {
    ...
    if (sd_char == '\n' )  
    {
      ...
      if (sd_count != 0)
      {
        ...
        infoCmd.queue[infoCmd.index_w].gcode[sd_count] = 0;  
  ...
  if ((infoPrinting.cur >= infoPrinting.size) && isPrinting())  
  {
    printingFinished();
  }
}
