  ...
  else
  {
    ...
    for (; configFile.cur < configFile.size;)
    {
      ...
      else if (count < maxLineLen - 2)
      {
        ...
        {
          if (comment_space && cur_char != ' ')  
            comment_space = false;
          if (!comment_mode && !comment_space && cur_char != '\r')  
          {
            if (cur_char == 'n' && last_char == '\\')  
            {
              cur_char = '\n';
              count--;
            }
...
{
  uint8_t r = ((rgb888 >> 16) & 0xFF) >> 3;  
  uint8_t g = ((rgb888 >> 8) & 0xFF) >> 2;   
  uint8_t b = (rgb888 & 0xFF) >> 3;          
  return ((r << 11) | (g << 5) | b);
}
...
void parseConfigKey(uint16_t index)
{
  switch (index)
  {
    ...
    #ifdef WUXN_CODE
    case C_INDEX_ENABLE_OOBWCW:
      infoSettings.enable_oobwcw = getOnOff();
      break;
    #endif
    ...
    case C_INDEX_CUSTOM_LABEL_15:
    {
      ...
      if (inLimit(utf8len, NAME_MIN_LENGTH, MAX_GCODE_NAME_LENGTH) && inLimit(bytelen, NAME_MIN_LENGTH, MAX_GCODE_LENGTH))
      {
        ...
        customcode_good[index - C_INDEX_CUSTOM_LABEL_1] = 1;  
      }
      else
      {
        customcode_good[index - C_INDEX_CUSTOM_LABEL_1] = 0;  
      }
      break;
    }
    ...
    {
      int fileindex = index - C_INDEX_CUSTOM_GCODE_1;  
      char pchr[LINE_MAX_CHAR];
      strcpy(pchr,strrchr(cur_line, ':') + 1);
      ...
      else if (customcode_good[fileindex] == 1)  
        customcode_index--;
      break;
    }
