...
void loopTouchScreen(void)  // Handle in interrupt
{
  ...
  #define TOUCH_CYCLES_TO_PRESSED  50 // 012322 (was 20ms in original code) hot long does the touch have to last to count as true?
  // 012322 each time touch is set to 0, hold off for HOLD_OFF_CYCLES before
  //        allowing touch to be incremented again.  This is done to reduce 
  //        the chances of a double-touch
 
  if (!XPT2046_Read_Pen())
  {
    if (touch >= TOUCH_CYCLES_TO_PRESSED)  // originally was 20ms
                      // 12/23/21 Issue 28 Priority 1 changed to 30 from 20
    {
