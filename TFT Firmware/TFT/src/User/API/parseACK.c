...
bool f_ReadyToPrintFilamentPresent = true;

const MENUITEMS Printing_blankMenuItems = {
...
const ECHO knownEcho[] = {
    ...
    {ECHO_NOTIFY_NONE, "echo:;"},
    {ECHO_NOTIFY_NONE, "echo:  G"},
    {ECHO_NOTIFY_NONE, "echo:  M"},
    {ECHO_NOTIFY_NONE, "Cap:"},
    {ECHO_NOTIFY_NONE, "Config:"},
    {ECHO_NOTIFY_TOAST, "Settings Stored"},
    {ECHO_NOTIFY_TOAST, "echo:Bed"},
#ifdef WUXN_CODE
    {ECHO_NOTIFY_NONE, "echo:Fade"},
#else
    {ECHO_NOTIFY_TOAST, "echo:Fade"},
#endif
    {ECHO_NOTIFY_TOAST, "echo:Active Extruder"},
    {ECHO_NOTIFY_NONE, "Unknown command: \"M150"},
};
  ...
  *find = '\0';
  ...
  if (ack_seen(":notification "))
  {
        ...
        ack_seen("Accuracy"))
    {
      ...
      if (ack_seen("Accuracy"))
        Buzzer_play(sound_success);
    }

    else if (!ack_seen("Ready."))
      addToast(DIALOG_TYPE_INFO, dmaL2Cache + index);
  }
  ...
  else if (ack_seen(":paused") || ack_seen(":pause"))
  {
    ...
    if (ack_seen("filament_runout") ||
        ack_seen("FilamentRunout") ||
        ack_seen("Filament Runout"))
    {
      setRunoutAlarmTrue();
      f_RunningM600 = true;
      menuDrawPage(&Printing_blankMenuItems);
    }
  }
  ...
  else if (ack_seen(":prompt_begin "))
  {
    ...
    {
      if (isPrinting() && (infoFile.source >= BOARD_SD))
...
void parseACK(void)
{
  ...
  while (dmaL1NotEmpty(SERIAL_PORT))
  {
    ...
    if (infoHost.connected == false)
    {
      ...
      if (infoMachineSettings.firmwareType == FW_NOT_DETECTED)
      {
        storeCmd("M503\n");
        storeCmd("M92\n");
    ...
    if (ack_seen("wait"))
    {
      if (isPrinting() && !isPause() && infoHost.wait == true)
      {
        infoHost.wait = false;
      }
    }
    ...
    else
    {
      ...
      else if ((ack_seen("X:") && ack_index == 2) || ack_seen("C: X:"))
      {
      ...
      else if (!infoMachineSettings.promptSupport && ack_seen(" for user"))
      {
      ...
      else if (ack_seen("Soft endstops"))
      {
        ...
        if (curValue != infoMachineSettings.softwareEndstops)
          addToast(DIALOG_TYPE_INFO, dmaL2Cache);
      }
      ...
      {
        if (ack_seen("M200 S"))
          setParameter(P_FILAMENT_SETTING, 0, ack_value());
        if (ack_seen("T"))
        {
          if (ack_seen("T0 D"))
            setParameter(P_FILAMENT_SETTING, 1, ack_value());
          if (ack_seen("T1 D"))
            setParameter(P_FILAMENT_SETTING, 2, ack_value());
        }
        else
        {
          if (ack_seen("D"))
      ...
      else if (ack_seen("M906"))
      {
        ...
        else
        {
          ...
          setParameter(P_STEALTH_CHOP, X_STEPPER, 0);
          setParameter(P_STEALTH_CHOP, Y_STEPPER, 0);
          setParameter(P_STEALTH_CHOP, Z_STEPPER, 0);
          setParameter(P_STEALTH_CHOP, E_STEPPER, 0);
          setParameter(P_STEALTH_CHOP, E2_STEPPER, 0);
        }
        if (ack_seen("T"))
        {
          if (ack_seen("T0 E"))
      ...
      else if (ack_seen("FIRMWARE_NAME:"))
      {
        ...
        if (ack_seen("FIRMWARE_URL:"))
          string_end = ack_index - sizeof("FIRMWARE_URL:");
        else if (ack_seen("SOURCE_CODE_URL:"))
          string_end = ack_index - sizeof("SOURCE_CODE_URL:");
        else if ((infoMachineSettings.firmwareType == FW_REPRAPFW) && ack_seen("ELECTRONICS"))
          string_end = ack_index - sizeof("ELECTRONICS");

        infoSetFirmwareName(string, string_end - string_start);
        ...
        if (ack_seen("MACHINE_TYPE:"))
        {
          ...
          infoSetMachineType(string, string_end - string_start);
        }
      }
      ...
      {
#ifdef WUXN_CODE
        if (ack_seen("Failed to enable Bed Leveling"))
        {
          if (infoSettings.factory_reset == DISABLE)
        ...
        else if (ack_seen("MINTEMP"))
        {

          if (ack_seen("bed"))
            wDisplayThermalErrorDialog((uint8_t *)"Hot Bed Thermistor Failure");
          else
            wDisplayThermalErrorDialog((uint8_t *)"Nozzle Thermistor Failure");
        }

        else if (ack_seen("MAXTEMP"))
        {

          if (ack_seen("bed"))
            wDisplayThermalErrorDialog((uint8_t *)"Hot Bed Over Temperature");
          else
            wDisplayThermalErrorDialog((uint8_t *)"Nozzle Over Temperature");
        }

        else if (ack_seen("Heating failed"))
        {

          if (ack_seen("bed"))
            wDisplayThermalErrorDialog((uint8_t *)"Hot Bed Heater Failure");
          else
            wDisplayThermalErrorDialog((uint8_t *)"Nozzle Heater Failure");
        }

        else if (ack_seen("Thermal Runaway"))
        {

          if (ack_seen("bed"))
            wDisplayThermalErrorDialog((uint8_t *)"Hot Bed Thermal Runaway");
          else
            wDisplayThermalErrorDialog((uint8_t *)"Nozzle Thermal Runaway");
        }
        else
        {
          ackPopupInfo(errormagic);
      ...
      else if (ack_seen("filament: TRIGGERED"))
      {
        f_ReadyToPrintFilamentPresent = true;
        wSetZTriggerCheckFlag(TRUE);
      }

      else if (ack_seen("filament: open"))
      {
        f_ReadyToPrintFilamentPresent = false;
        wSetZTriggerCheckFlag(FALSE);
      }
#endif
      ...
      else if (infoMachineSettings.firmwareType == FW_REPRAPFW)
      {
        ...
        else if (ack_seen("access point "))
        {
          ...
          infoSetAccessPoint(string, string_end - string_start);
          ...
          if (ack_seen("IP address "))
          {
            ...
            if (ack_seen("\n"))
              string_end = ack_index - 1;
            infoSetIPAddress(string, string_end - string_start);
          }
        }
      }
      else if (infoMachineSettings.firmwareType == FW_SMOOTHIEWARE)
      {
        if (ack_seen(errorZProbe))
        {
