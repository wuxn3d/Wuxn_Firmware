...
void GcodeSuite::G34() {
  ...
  do { // break out on error
    ...
    while (iteration < z_auto_align_iterations) {
      ...
      auto decreasing_accuracy = [](const float &v1, const float &v2, const int &iter_value){  // as modified by Wuxn Trensor, added ", const int &iter_value"
        if (v1 < v2 * 0.7f) {
          SERIAL_ECHOLNPGM("Decreasing Accuracy Detected.");
          LCD_MESSAGEPGM(MSG_DECREASING_ACCURACY);
          return true;
        }
        else if (((v2 - v1) > 0.2) && (iter_value > 0)){   // added by Wuxn Trensor
          SERIAL_ECHOLNPGM("Decreasing Accuracy Detected.");  // added by Wuxn Trensor
          return true;  // added by Wuxn Trensor
        }
        return false;
      };
      ...
      LOOP_L_N(zstepper, NUM_Z_STEPPER_DRIVERS) {
        ...
        const float z_align_abs = ABS(z_align_move);
          ...
          if (decreasing_accuracy(last_z_align_move[zstepper], z_align_abs, iteration)) {  // modified by Wuxn Ternsor (added ", iteration")
