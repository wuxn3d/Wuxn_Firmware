...
void GcodeSuite::M106() {
  ...
  // For Wuxn Wizards, send "M106 P9 S255" for Hotend Fan
  // to start. There is an inbuilt thermal control feature
  // will shutdown the fan automatically if the hotend
  // Temperature is below threshold level.
  #ifdef E0_AUTO_FAN_PIN
  #define WUXN_WIZARD_HOTENDFAN_TEST (9)
  if(pfan == WUXN_WIZARD_HOTENDFAN_TEST)
  {
    uint16_t speed = 0;
    if(parser.seenval('S')) {
      speed = parser.value_ushort();
      if(speed <= 255) {
        analogWrite(pin_t(E0_AUTO_FAN_PIN), speed);
      }
    }
  }
  #endif
